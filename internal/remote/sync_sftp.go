...
func uploadSftp() error {
	...
	// init sftp client
	sftpUploader, err := uploader.NewUploader(uploader.SftpUploaderType, cfg.Upload)
	if err != nil {
		return err
	}
...
func deleteOnRemote(sftpUploader uploader.Uploader) error {
	cfg := config.Cfg()
	sftpConfig := cfg.Upload.Sftp
...
func getFilesToUpload(uploader uploader.Uploader) ([]uploadTask, error) {
	cfg := config.Cfg()
	sftpConfig := cfg.Upload.Sftp
...
func uploadOnRemote(sftpUploader uploader.Uploader) error {
	// prepare tasks
...
func uploadWorker(worker int, uploader uploader.Uploader, tasks <-chan uploadTask, wg *sync.WaitGroup) {
	defer wg.Done()
